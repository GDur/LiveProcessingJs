...
class Circle
{
	...
	void move(dt)
	{
		...
		pos.x =0+ startPos.x + tmpG * currentTime * currentTime / 2 + vel.x * currentTime; 
		pos.y = r;
		if(pos.x > theSize.x)
			currentTime = 0;
	}
...
// Aufgabe 3.
Circle circle = new Circle(new v2(0, 10), .25);
float dt = 0;
float t;
